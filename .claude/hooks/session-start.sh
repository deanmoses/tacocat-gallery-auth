...
#
# The Node version is the point. package.json sets engines.node to 24, and the
# cloud image ships an older Node. npm only warns about the engine mismatch and
# installs anyway, which does quiet damage: the npm bundled with Node 22
# rewrites package-lock.json, stripping the `libc` fields that newer npm records
# for optional platform-specific packages, so an unrelated change arrives with a
# few dozen lines of lockfile churn.
#
...
# install can reuse what is already unpacked there.
step_start "npm-install"
npm install --no-audit --no-fund
step_done

echo "[session-start] done total=$((SECONDS - script_start))s"
echo "Session ready: npm test, npm run lint and npm run typecheck should all work."
